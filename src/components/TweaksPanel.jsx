/* ============================================================
   ...
   Adapted from the Claude Design "omelette" starter scaffold for a
   standalone app: the host edit-mode postMessage protocol is replaced
   by a self-contained floating launcher + local React state.
...
const __TWEAKS_STYLE = `
  .twk-launcher{position:fixed;top:16px;right:16px;z-index:2147483646;
    ...
    font:600 12.5px/1 ui-sans-serif,system-ui,-apple-system,sans-serif}
  .twk-launcher:hover{background:rgba(250,249,247,.95)}
  .twk-launcher svg{width:15px;height:15px}

  .twk-panel{position:fixed;top:16px;right:16px;z-index:2147483646;width:280px;
...
// ── useTweaks ───────────────────────────────────────────────────────────────
// Single source of truth for tweak values (local React state).
...
// ── TweaksPanel ─────────────────────────────────────────────────────────────
// Floating shell with its own launcher (no design-host required).
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  return (
    <>
      ...
      {!open && (
        <button className="twk-launcher" onClick={() => setOpen(true)} aria-label="Open tweaks">
          <svg viewBox="0 0 16 16" fill="none" aria-hidden="true">
            <path d="M2 4.5h7M11.5 4.5H14M2 11.5h2.5M7 11.5h7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            <circle cx="10" cy="4.5" r="1.9" fill="currentColor"/>
            <circle cx="5.5" cy="11.5" r="1.9" fill="currentColor"/>
          </svg>
          {title}
        </button>
      )}
